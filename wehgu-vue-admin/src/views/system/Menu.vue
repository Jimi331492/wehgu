 ...
 * @LastEditTime: 2022-04-26 15:39:34
      ...
      <el-table :data="permList" border stripe default-expand-all row-key="permId" :tree-props="{ children: 'children' }" :indent="8">
        ...
        <el-table-column prop="type" label="类型">
          <template v-slot="scope">
            ...
            <el-tag type="warning" v-else-if="scope.row.parentId === null">根</el-tag>
        ...
        <el-table-column label="权限等级">
          <template v-slot="scope">
            ...
            <el-tag type="warning" v-else-if="scope.row.parentId === null">顶级</el-tag>
        ...
        <el-table-column label="操作" width="200px">
          <template v-slot="scope">
            <!-- 编辑按钮 -->
            <el-tooltip v-if="scope.row.parentId !== null" class="item" effect="dark" content="编辑" placement="top" :enterable="false">
              <el-button type="primary" icon="el-icon-edit" size="mini" @click="showEditDialog(scope.row)"></el-button>
            </el-tooltip>

            <!-- 删除按钮 -->
            <el-tooltip v-if="scope.row.parentId !== null" class="item" effect="dark" content="删除" placement="top" :enterable="false">
    ...
    <el-dialog :title="titles" v-model="dialogVisible" @close="dialogClosed" width="60%">
      <el-form ref="formRef" :model="form" :rules="rules" label-width="5rem">
        ...
        <el-form-item label="父级菜单:" prop="parentId" v-show="form.type === '0'">
          <el-cascader v-model="form.parentId" :options="menuPermList" :props="menuProps" @change="handleChange" />
        </el-form-item>
...
export default {
  ...
  methods: {
    ...
    async queryPermPage() {
      ...
      //过滤按钮基本权限
      let menuPermList = this.$_.cloneDeep(res.data.records)
      menuPermList = this.filterTree(menuPermList)
      //过滤children为空的权限
      this.menuPermList = this.menusIsBlank(menuPermList)
      console.log(this.menuPermList)
    },
